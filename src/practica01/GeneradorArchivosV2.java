...
public class GeneradorArchivosV2 {
	...
	public static String matrizEmpresa() {
		StringBuilder salida = new StringBuilder("%@MATRIZ_EMPRESAS\n%");
		char[] s;
		String[] s2;
		s = new char[26];
		s2 = new String[702];
		...
		if (avenida % 2 != 0) {
			for (int i = aPrincipal - 1; i >= 0; i--) {
				for (int j = 1; j <= calle; j++) {
					salida.append(s2[i]);
					if (i == 0) {
						salida.append(j).append(" ");
					} else {
						salida.append("N").append(j).append(" ");
					}
		...
		for (int i = 1; i <= aPrincipal; i++) {
			for (int j = 1; j <= calle; j++) {
				salida.append(s2[i]);
				salida.append("S").append(j).append(" ");
			}
			salida.append("\n%");
		}
//		System.out.println(salida);
	...
	public static String matrizSensor() {
		StringBuilder salida = new StringBuilder("%@MATRIZ_SENSORES\n%");
		char[] s;
		String[] s2;
		s = new char[26];
		s2 = new String[702];
		...
		for (int i = 1; i < calles + 2; i++) {
			salida.append("A").append(i).append(" ");
		}
		salida.append("\n%");
	...
	public static String listado() {
		...
		for (int i = (aPrincipal - 1); i >= 0; i--) {
			for (int j = 1; j <= calle; j++) {
				...
				salida.append(formatoFlujoEmpresa.format(flujoEmpresa)).append(" - ");
				...
				for (int z = 0; z < 2; z++) {
					...
					salida.append(formatoContaminante.format(flujoEmpresa)).append(";");
				}
				salida.append("\n");
			}
			salida.append("\n");
